...
import { actualizarInformacionPersonal, eliminarImagenInformacionPersonal } from "../database/informacionPersonal";
import { alertaError, alertaPreguntaEliminar, alertaSuccess } from "../helpers/alertas";
import { useForm } from "../hooks/useForm";
import { Imagen } from "./Imagen";

export const InformacionInfPersonal = ({
  informacionPersonal,
  editar,
  refrescar,
  setRefrescar,
  ...
  const initialState = {
    titulo: informacionPersonal.titulo,
    descripcion: informacionPersonal.descripcion,
  };
  const { uid } = informacionPersonal;
  ...
  const handleSubmit = async (e) => {
    ...
    const resp = await actualizarInformacionPersonal(uid, data);
  ...
  const eliminarImagen=async()=>{
    ...
    try {
      ...
      if (seEliminara) {
        const resp = await eliminarImagenInformacionPersonal(uid);
            ...
            <div className="d-flex justify-content-center flex-column align-items-center">
              {editar && (
                <div className="d-flex justify-content-center mt-4 position-relative mb-5">
                  ...
                  >
                    {informacionPersonal.img.length <= 0
                      ? "SUBIR IMAGEN"
                      : "REMPLAZAR IMAGEN"}
                  </button>
                </div>
              ...
              {informacionPersonal.img.length > 0 && (
                <Imagen editar={editar} src={informacionPersonal.img} eliminar={eliminarImagen} />
              )}
            </div>
